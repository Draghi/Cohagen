...
typedef enum {GAME_STATE, QUIT_STATE, LIGHTING_STATE} stateType;
static stateType gameState = GAME_STATE;
...
typedef struct NewtonsCradleData_s {
	...
	Shader *passThruShader;
	Texture *quitScreenTexture;
...
static void initGameObjRegist(GameLoop* self) {
	...
	manGameObjRegist.setShader(data->gameObjRegist, manShader.newFromGroup("./data/shaders/", "passThru"));
...
static void initQuitScreen(GameLoop *self) {
	NewtonsCradleData *data = (NewtonsCradleData*)self->extraData;

	data->passThruShader = manShader.newFromGroup("./data/shaders/", "passThruTex");
	data->quitScreen = manRenderObj.new(NULL, NULL, NULL);

	int posLoc = manShader.getAttribLocation(data->passThruShader, "vPos");
	int normLoc = manShader.getAttribLocation(data->passThruShader, "vNorm");
	int texLoc = manShader.getAttribLocation(data->passThruShader, "vTex");
	...
	// Load texture
	data->quitScreenTexture = textureUtil.createTextureFromFile("./data/texture/quitScreen.bmp", GL_LINEAR, GL_LINEAR);

	glUseProgram(data->passThruShader->program);
	glUniform1i(glGetUniformLocation(data->passThruShader->program, "tex"), 1);
	glUseProgram(0);
}
...
static void update(float tickDelta, Window* window, Vec3* camPos, Vec3* camRot, GameObjectRegist* gameObjRegist) {
	...
	if (gameState == GAME_STATE)
	{
		...
		if (manKeyboard.isDown(window, KEY_Q))
		{
...
static void renderQuitScreen(float frameDelta, Window *window, Shader *quitScreenShader, RenderObject *quitScreen, GameObjectRegist *gameObjRegist, MatrixManager *manMat, Texture *quitScreenTexture)
{
	glViewport(0, 0, manWin.getFramebufferWidth(window), manWin.getFramebufferHeight(window));
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	...
	// MatrixManager* matMan = gameObjRegist->matMan;
	// manMatMan.setMode(matMan, MATRIX_MODE_VIEW);
	// 	manRenderer.renderModel(quitScreen, quitScreenShader, matMan);
	// manMatMan.push(matMan);
	// manMatMan.setMode(matMan, MATRIX_MODE_VIEW);
	// manMatMan.pop(matMan);
	...
	manMatMan.push(manMat);
		manMatMan.setMode(manMat, MATRIX_MODE_MODEL);
		manMatMan.push(manMat);
			manShader.bind(quitScreenShader);
				manTex.bind(quitScreenTexture, GL_TEXTURE_2D, 1);
					manVAO.bind(quitScreen->model);
						manVAO.draw(quitScreen->model);
					manVAO.unbind();
				manTex.unbind(quitScreenTexture, GL_TEXTURE_2D);
			manShader.unbind();
...
static void onRender(GameLoop* self, float frameDelta) {
	...
	switch (gameState)
	{
		...
		break;
		case QUIT_STATE:
			renderQuitScreen(frameDelta, self->primaryWindow, data->passThruShader, data->quitScreen, data->gameObjRegist, data->manMat, data->quitScreenTexture);
		break;
	}
}
